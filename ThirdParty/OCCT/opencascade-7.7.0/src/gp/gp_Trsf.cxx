...
// DI_MODIFICATION Added
Standard_Boolean gp_Trsf::IsIdentity(Standard_Boolean ignoreLocation) const
{
    if (!ignoreLocation && loc.SquareModulus() > gp::Resolution())
        return Standard_False;

    if (fabs(scale - 1.) > gp::Resolution())
        return Standard_False;
    ...
    for (Standard_Integer i = 1; i <= 3; i++)
    {
        ...
        {
            if (fabs(I.Value(i, j)) > gp::Resolution())
                return false;
        }
    }
