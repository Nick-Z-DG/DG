...
// DI_MODIFICATION Added the function. If number of local values is different, checks that the difference is identity
Standard_Boolean TopLoc_Location::IsEqual2(const TopLoc_Location& Other, Standard_Boolean ignoreLocation) const
{
	const void** p = (const void**)&myItems;
	const void** q = (const void**)&Other.myItems;
	if (*p == *q) { return Standard_True; }
	...
	{
		const TopLoc_Location* pLocnNonIdentity = (IsIdentity() ? &Other : this);
		gp_Trsf transf = pLocnNonIdentity->Transformation();
		return transf.IsIdentity(ignoreLocation);
	}

	if (FirstDatum() != Other.FirstDatum()) { return Standard_False; }
	if (FirstPower() != Other.FirstPower()) { return Standard_False; }

	const TopLoc_Location& locThis = NextLocation();
	const TopLoc_Location& locOther = Other.NextLocation();

	return locThis.IsEqual2(locOther, ignoreLocation);
}
