...
//=======================================================================
// DI_MODIFICATION cleaned and commented the function. Scope brackets style changed. Tabs increased to 4 and other improvements
// to clarify this often used and complicated code
//function : Orientation
//purpose  : Checks validity of orientation of wires in face F
//=======================================================================
BRepCheck_Status BRepCheck_Wire::Orientation(const TopoDS_Face& F, const Standard_Boolean Update)
{
    ...
    TopTools_MapOfShape mapS;   // Already passed valid edges in the wire with valid orientation.
    TopoDS_Edge theEdge;        // The current edge. If everythign is valid and not degenerated the current vertex VL is the second vertex of it
    TopoDS_Edge theRef;

    // This for loop finds the first oriented edge with both vertices non-null. Copies its forward (the first) vertex to VF and the other to VL
    for (exp.Init(myShape,TopAbs_EDGE); exp.More(); exp.Next()) 
    {
        const TopoDS_Edge& edg = TopoDS::Edge(exp.Current());
        orient = edg.Orientation();

        if (IsOriented(edg))    // Has a valid orientation TopAbs_FORWARD or TopAbs_REVERSED ?
        {
            mapS.Add(edg);
            theEdge = edg;
            theRef = edg;
    ...
    if (theOstat != BRepCheck_NoError && Update)
    {
        if (Update)
        BRepCheck::Add(myMap(myShape), theOstat);
        return theOstat;
    ...
    // myMapVE is a map vertex to list of edges. myMapVE(index] - list of edges having the index-th vertex as an end
    ...
    // The main loop. Starting with the current vertex which is either VL (if not null) or VF find the next (in CCW direction if isGoFwd)
    // vertex linked to the current vertex by an edge.
    while (Index < nbOriNoDegen) 
    {
        ledge.Clear();
        ListOfPassedEdge.Clear();

        //Find edges that make a chain on VL if !VL.IsNull otherwise on VF.
        ...
        // Preliminary comment: It looks like this for loop is building ledge, which normally contains a single edge next in the chosen direction after and next 
        // to the current edge theEdge. theEdge and the found edge will be linked to (share) the current vertex (VL if all is good)
        //
        // Loop by edges having the current vertex as its end
        for (TopTools_ListIteratorOfListOfShape itls(myMapVE(ind)); itls.More(); itls.Next()) 
        {
            ...
            if (mapS.Contains(edg))
                ortmp = GetOrientation(mapS, edg);          //NZ. Does not seem to do anything. Looks like always returns edg.Orientation();

            //Add to list already passed outcoming edges
            if (mapS.Contains(edg) && ortmp == orient && !edg.IsSame(theEdge))  // Is edg different from the current theEdge?
            {
                for (vte.Init(edg, TopAbs_VERTEX); vte.More(); vte.Next())
                {
                    TopAbs_Orientation vto = vte.Current().Orientation();
                    ...
                    if (!VL.IsNull())
                    {
                        ...
                        {
                            // Case VL is the first vertex of edg 
                    ...
                    else // VF is not null
                    {
                        ...
                        {
                            // Case VF is the second vertex of edg 
                            ListOfPassedEdge.Append(edg);
                            break;
                        }
                    }
            ...
            if (!mapS.Contains(edg) || ortmp != orient) 
            {
                for (vte.Init(edg,TopAbs_VERTEX);vte.More(); vte.Next()) 
                {
                    ...
                    if (!VL.IsNull()) 
                    {
                        if (vto == TopAbs_FORWARD && VL.IsSame(vte.Current())) 
                        {
                            // If the processing is in 2d (face not null) or if the edge is not degenerated it is added
        ...
        Standard_Boolean Changedesens = Standard_False;     // Changed direction of stepping along the wire?

        if (nbconnex == 0) 
        {
            // Case no edges coming out of the current vertex
        ...
        {
            TopoDS_Vertex pivot;        // The current vertex
            if (!VL.IsNull()) 
              pivot = VL;
            else 
              pivot = VF;

            ChoixUV(pivot, theEdge, F, ledge);            // Update ledge list of out edges using geometric calculations
        ...
        if (Index==1 && myCstat!=BRepCheck_NotClosed && isCheckClose && !F.IsNull()) 
        {
	        ...
	        ChoixUV(aVRef, theRef, F, ledge);            // Update ledge list of out edges using geometric calculations
