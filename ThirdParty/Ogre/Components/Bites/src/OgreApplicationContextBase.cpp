...
void ApplicationContextBase::locateResources()
{
    ...
    Ogre::String binFolder = mFSLayer->getConfigPath(0); // DI_MODIFICATION
    ...
    for(seci = cf.getSettingsBySection().begin(); seci != cf.getSettingsBySection().end(); ++seci) {
        ...
        for (i = settings.begin(); i != settings.end(); i++)
        {
            ...
            Ogre::String::size_type iCol = arch.find(':', 0);
    ...
    {
        const auto& mediaDir = binFolder + getDefaultMediaDir(); // DI_MODIFICATION
...
void ApplicationContextBase::shutdown()
{
    ...
    if (mOverlaySystem)
    {
        ...
        mOverlaySystem = nullptr; // DI_MODIFICATION
    }
